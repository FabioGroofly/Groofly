...
page = requests.get("http://www.paragear.com/parachutes/10000070/MAIN-PARACHUTESS")
...
#        print (page.status_code)
        ...
        mainParachute.append(CanopyName.replace("PARACHUTE","").replace("PD","").replace(nameManuf.upper(),"").strip())
##
a = pd.DataFrame({"Manufacture":Manuf})
b = pd.DataFrame({"Main Canopy":mainParachute})
##c = pd.DataFrame({"Tandem Canopy":tandemParachute})
#####
Catalog = pd.concat([a, b], ignore_index=False, axis=1)
Catalog.to_csv('List-Manufacture&Canopies.csv', index=False)